...
export class AureliaD3TimelineCustomElement {
  ...
  private draw() {
    ...
    const markers = tl
      ...
      .on('mouseover', d => {
        tooltip.transition()
          .duration(200)
          .style('opacity', .9);
        tooltip.html(formatTime(d.start) + '<br/>' + d.label)
    ...
    const zoomed = () => {
      ...
      const xZoomed = currentEvent.transform.rescaleX(x);
      gX.call(xAxis.scale(xZoomed));
      // markers.attr('transform', `translate(${currentEvent.transform.x}, 0) scale(${currentEvent.transform.k}, 1)`);
      svg
        .selectAll('circle')
        .attr('cx', d => xZoomed(d.start));
    };
    const zoom = d3.zoom()
      // .scaleExtent([1, 40])
